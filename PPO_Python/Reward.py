"""V15 reward: progress, deterioration penalties, and exhaustion protection."""
...
def Reset(env, obs):
    ...
    env.previous_stamina_diagnostic = float(obs["Stamina"])
...
def Reward(obs, act, env):
    ...
    total_bleed_speed_delta_reward = (
        -TOTAL_BLEED_SPEED_DELTA_REWARD_SCALE * max(0.0, total_bleed_speed_delta)
    )
    ...
    internal_bleeding_delta_reward = (
        -INTERNAL_BLEEDING_DELTA_REWARD_SCALE * max(0.0, internal_bleeding_delta)
    )
    ...
    radiation_sickness_delta_reward = (
        -RADIATION_SICKNESS_DELTA_REWARD_SCALE * max(0.0, radiation_sickness_delta)
    )
    ...
    sickness_amount_delta_reward = (
        -SICKNESS_DELTA_REWARD_SCALE * max(0.0, sickness_amount_delta)
    )
    ...
    temperature_delta_reward = (
        -TEMPERATURE_DELTA_REWARD_SCALE * max(0.0, temperature_deviation_delta)
    )
    ...
    env.previous_stamina_diagnostic = stamina
    ...
    env.last_reward_terms = {
        ...
        "diagnostic_stamina_delta": stamina_delta_diagnostic,
